...
const VariantFormRow = ({
  ...
  const handleVariantAttributeSearch = (e: any) => {
    ...
    const filteredCategoryList = filtered.map((item: VariantAttributeT) => ({
      value: item.name,
      label: item.name,
    }));

    setFilteredVariantItems(filteredVariantItems);
  }
        ...
        <Row gutter={20}>
          <Col span={addVariantRows.length < 2 ? 12 : 11}>
            <FormItem>
              ...
              // initialValue={variantTypeSearchTerm}
              >
                <Select
                  ref={selectRef}
                  style={{ height: 56.5 }}
                  ...
                  onSelect={handleBlur}
                  options={variantItems.map((item) => (filteredVariantItems))}
                  ...
                  dropdownRender={(menu) => (
                    <div style={{ padding: '5px' }}>
                      ...
                      {variantTypeSearchTerm ? (
                        ...
                        >
                          Add "{variantTypeSearchTerm}" as new category
                        </Button>
                      ) : ("")}
                    </div>
                  )}
